            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left min-w-[800px]">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tr x-show="matchesSearch('{{ addslashes($task->title) }}', '{{ addslashes($task->description) }}')" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <td class="p-5">
                                <div class="font-bold text-gray-900 dark:text-gray-100 text-lg {{ $task->status === 'Completed' ? 'line-through opacity-60' : '' }}">{{ $task->title }}</div>
                                <p class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-md {{ $task->status === 'Completed' ? 'line-through opacity-60' : '' }}">{{ $task->description }}</p>
                            </td>
                            <td class="p-5">
                                ...
                                <div class="text-[10px] text-gray-400 dark:text-gray-500">Ends: {{ \Carbon\Carbon::parse($task->due_date)->format('M d, Y') }}</div>
                            </td>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="priority" x-model="currentTask.priority" class="w-full bg-transparent border border-gray-300 dark:border-gray-600 dark:text-white rounded-xl p-3 text-sm focus:ring-2 ring-indigo-500 outline-none">
                                <option value="Low" class="dark:bg-gray-800">Low Priority</option>
                                <option value="Medium" class="dark:bg-gray-800">Medium Priority</option>
                                <option value="High" class="dark:bg-gray-800">High Priority</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status" x-model="currentTask.status" class="w-full bg-transparent border border-gray-300 dark:border-gray-600 dark:text-white rounded-xl p-3 text-sm focus:ring-2 ring-indigo-500 outline-none">
                                <option value="Pending" class="dark:bg-gray-800">Pending</option>
                                <option value="Completed" class="dark:bg-gray-800">Completed</option>
                            </select>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function taskManager() {
            return {
                ...
                init() {
                    // Watch for changes to darkMode and save to localStorage
